...
class OpenGLCubeApp {
 ...
 private:
  ...
  void mainLoop() {
    ...
    LOG("Camera position: (" << cameraPos.x << ", " << cameraPos.y << ", "
                             << cameraPos.z << ")");
    ...
    glfwSetKeyCallback(window, [](GLFWwindow* win, int key, int scancode,
      ...
      (void)mods;
      auto* app =
          reinterpret_cast<OpenGLCubeApp*>(glfwGetWindowUserPointer(win));
      if (!app) return;
    ...
    // auto sphere1 = std::make_unique<primitives::Sphere>(1.0f, 64, 128,
    // shader); sphere1->transform.position = {0, 0, 0};
    // sphere1->transform.dirty = true;
    // sphere1->color = {1, 1, 0, 1};
    // scene.push_back(std::move(sphere1));

    // auto cube1 =
    // std::make_unique<primitives::Cube>(glm::vec3(1.0f, 1.0f, 1.0f), shader);
    ...
    while (!glfwWindowShouldClose(window)) {
      ...
      if (keys[GLFW_KEY_W]) cameraPos += forward * v;
      if (keys[GLFW_KEY_S]) cameraPos -= forward * v;
      if (keys[GLFW_KEY_A]) cameraPos -= right * v;
      if (keys[GLFW_KEY_D]) cameraPos += right * v;
