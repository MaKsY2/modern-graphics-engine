...
namespace primitives
{
    ...
    namespace
    {
        ...
        std::shared_ptr<utils::Mesh> createUnitSphereMesh(uint32_t stacks, uint32_t slices)
        {
    ...
    std::shared_ptr<utils::Mesh> Sphere::getCachedMesh(uint32_t stacks, uint32_t slices)
    {

        std::vector<Entry> cache;

        for (auto &e : cache)
            if (e.st == stacks && e.sl == slices)
                return e.mesh;
    ...
    Sphere::Sphere(float radius, uint32_t stacks, uint32_t slices, const std::shared_ptr<Shader> &shader)
