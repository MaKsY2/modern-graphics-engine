...
glm::mat4 nodeLocalMatrix(const tinygltf::Node& n) {
  ...
  if (n.translation.size() == 3)
    t = {toFloat(n.translation[0]), toFloat(n.translation[1]),
         toFloat(n.translation[2])};
  if (n.scale.size() == 3)
    s = {toFloat(n.scale[0]), toFloat(n.scale[1]), toFloat(n.scale[2])};
  if (n.rotation.size() == 4) {
    // glTF: x y z w
    q = glm::quat(toFloat(n.rotation[3]), toFloat(n.rotation[0]),
                  toFloat(n.rotation[1]), toFloat(n.rotation[2]));
  }
...
const std::byte* accessorBasePtr(const tinygltf::Model& model,
  ...
  const size_t offset = static_cast<size_t>(view.byteOffset) +
...
std::uint32_t readIndexScalar(const std::byte* base, size_t stride,
                              size_t index, int componentType) {
...
void appendPrimitivePU(const tinygltf::Model& model,
  ...
  if (accUv) {
    if (accUv->componentType != TINYGLTF_COMPONENT_TYPE_FLOAT ||
        accUv->type != TINYGLTF_TYPE_VEC2)
      throw std::runtime_error(
          "TEXCOORD_0 must be FLOAT VEC2 (minimal loader)");
  }
  ...
  for (size_t i = 0; i < accPos.count; ++i) {
    ...
    if (uvBase) {
      ...
      if (flipV) uv.y = 1.0f - uv.y;  // часто нужно для OpenGL
    }
  ...
  for (size_t k = 0; k < accIdx.count; ++k) {
    const std::uint32_t idx =
        readIndexScalar(idxBase, idxStride, k, accIdx.componentType);
    out.indices.push_back(baseVertex + idx);
  }
}

void processNodePU(const tinygltf::Model& model, int nodeIndex,
                   const glm::mat4& parent, LoadedMeshPU& out, bool flipV) {
...
LoadedMeshPU LoadGLB_ToCPU_PU(const std::string& path, bool flipV) {
  ...
  if (!ok) throw std::runtime_error("LoadGLB failed: " + err);
